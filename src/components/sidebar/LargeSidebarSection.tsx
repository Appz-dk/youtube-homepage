...
export const LargeSidebarSection = ({ title, visibleItemsCount = Number.POSITIVE_INFINITY, items, isActive }: LargeSidebarSectionPorps) => {
  ...
  return (
    <div className="border-b p-2">
      ...
      {visibleItems.map(i => (
        <LargeSidebarItem key={i.title} isActive={i.title === isActive} IconOrImgUrl={i.IconOrImgUrl} title={i.title} url={i.url} />
      ))}
      {showExpandBtn && (
        <Button
          variant="ghost" 
